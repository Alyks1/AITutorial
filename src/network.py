...
class Network(object):
    ...
    def backprop(self, input, expected):
        ...
        # backward pass
        # we want DC / D(w,b) so we do Dz / D(w,b) * DC / Da * Da / Dz
        # DC / Da * Da / Dz
        delta = self.cost_derivative(activations[-1], expected) * sigmoid_derivative(zs[-1])
        # Dz / Db = 1 * DC / Da * Da / Dz
        nabla_b[-1] = delta
        # Dz / Dw = activation of previous layer * DC / Da * Da / Dz
        nabla_w[-1] = np.dot(delta, activations[-2].transpose())

        for l in range(2, self.num_layers):
            z = zs[-l]
            sd = sigmoid_derivative(z)
            # Dz / Da = weights infront
            # we want the DC / Da-1 = Dz / Da-1 * Da / dz * DC / Da
    ...
    def cost_derivative(self, output_activations, expected):
        return 2 * (output_activations - expected)
